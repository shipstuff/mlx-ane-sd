"""Phase F.1 contention (external-cache variant).

Same shape as phaseF1_contention.py and phaseF0_contention.py but uses
the external-cache ANE DFlash (100% ANE placement).
"""
...
def main():
    ...
    if args.mode == "parallel":
        print(f"[main] bg workload: {args.bg_model}")
        ...
        if not ready_event.wait(timeout=120):
            print("[main] bg failed to load"); sys.exit(1)
        time.sleep(3)

    try:
        from huggingface_hub import snapshot_download
        from dflash_torch import DFlashConfig
        from phaseF1_ane_stream_ext import DFlashANEExtDraft, stream_generate_ane_ext
        ...
        print("[load] draft (ANE external-cache)...")
        ...
        print(f"mean: {statistics.mean(tpss):.2f} tok/s "
