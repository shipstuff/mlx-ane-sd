"""Phase F.1 contention: ANE DFlash under GPU-side background workload.

Checks whether ANE offload of the DFlash draft preserves more solo throughput
under contention than GPU-only DFlash. This is the Phase C pattern applied
to the DFlash pair. Even the stateless F.1 model should show this
preservation because the draft compute is fully off the GPU.
"""
...
def background_load_worker(model_name: str, stop_event, ready_event):
    ...
    ready_event.set()
    print(f"[bg] ready", flush=True)

    prompts = ["Explain neural networks.", "Write a haiku.",
    ...
    print(f"[bg] generated {total} tok in {elapsed:.1f}s ({total/elapsed:.1f} tok/s)",
          flush=True)
...
def main():
    ...
    ap.add_argument("--mlmodelc",
                    default="/tmp/dflash_ane_compiled/dflash_ane.mlmodelc")
    ...
    try:
        ...
        from phaseF1_ane_stream import DFlashANEDraft, stream_generate_ane
        ...
        for name, prompt in PROMPTS:
            print(f"\n=== {name} ({args.mode}) ===")
            gen, t, accepted, cycles, _ = stream_generate_ane(target, draft, tok,
                                                                prompt, args.max_new)
            tps = len(gen) / t
